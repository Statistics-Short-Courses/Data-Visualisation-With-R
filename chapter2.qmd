---
title: "How To Visualise Numerical Data"
format: 
  live-html:
    toc: true
resources: 
  - "Data-sets"
  
execute:
  echo: true
  warning: false
  message: false

webr:
  editor: true
  render-df: gt-interactive
    
embed-resources: true

filters:
  - custom-numbered-blocks

custom-numbered-blocks:
  classes:
    Assumption:
      colors: [FFCDD2, F44336]
      boxstyle: foldbox.simple
      collapse: false
    Example:
      colors: [BBDEFB, 2196F3]
      boxstyle: foldbox.simple
      collapse: false
    Exercise:
      colors: [C8E6C9, 4CAF50]
      boxstyle: foldbox.simple
      collapse: false
    Technical-point:
      colors: [FFF9C4, FFEB3B]
      boxstyle: foldbox.simple
      collapse: true
---

{{< include ./_extensions/r-wasm/live/_knitr.qmd >}}

{{< include ./_extensions/r-wasm/live/_gradethis.qmd >}}

```{r, echo=FALSE}
library(webexercises)
```

We have learnt how to visualise data using tables, which are useful for both analysts and readers when examining a dataset.

However, tables are not always the most effective way to quickly and intuitively understand patterns or trends in the data. This is where figures become valuable.

In this chapter, we will focus on different types of figures that can be used to represent quantitative variables. For plot interpretation, refer to the **Inferential Statistics With R** short course.

# Rice

![](images/zhao-yangjun-dDAzpSUAbgI-unsplash.jpg){width="455"}

*Photo by [Zhao Yangjun](https://unsplash.com/@youngjun420?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText) on [Unsplash](https://unsplash.com/photos/a-large-field-of-green-grass-with-a-sky-background-dDAzpSUAbgI?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText)*

In this chapter, we are going to explore the rice dataset.

```{r, message=FALSE, warning=FALSE, echo=FALSE}
library(knitr)
library(kableExtra)

variables <- data.frame(
  No = 1:23,
  Variable = c(
    "Ref", "ExpSite", "Year", "ExpName", "SowingMethod",
    "Range", "Row", "Rep", "Variety", "Nitrogen",
    "NGroup", "Estab", "PI_DM", "PI_Tillers", "PI_NIR",
    "PI_N_Uptake", "PM_DM", "HarvTillers", "HI",
    "QuadYield", "HeaderYield", "Lodging", "PlantHeight"
  ),
  Description = c(
    "Reference number",
    "Name of the site/farmer",
    "Rice sown in October and harvested the following year",
    "Combination of experiment site and year",
    "Four sowing methods – Drill, Aerial, Delayed Permanent Water (DPW) and DryBC",
    "Replicate identification number of the experiment (each treatment has 3 replicates)",
    "Row identification number (1–24)",
    "Same as Range",
    "Variety of rice",
    "Nitrogen rates (kg N/ha) applied prior to Permanent Water (PW) and at Panicle Initiation (PI)",
    "Groups representing different nitrogen rates",
    "Plant population at establishment (No./m²)",
    "Weight of dry matter at PI (g/m²)",
    "Count of tillers at PI (No./m²)",
    "Percentage of nitrogen in tissue at PI (% N)",
    "Nitrogen uptake at PI (kg N/ha)",
    "Weight of dry matter at physiological maturity (g/m²)",
    "Count of tillers at PM (No./m²)",
    "Harvest index (ratio of grain to total plant dry weight)",
    "Quad (hand-harvested) grain yield (t/ha at 14% moisture)",
    "Header (machine-harvested) grain yield (t/ha at 14% moisture)",
    "Lodging score at harvest (1 = standing, 10 = fully lodged)",
    "Average plant height at harvest (cm)"
  )
)

kable(
  variables,
  caption = "Description of Data Set Variables",
  align = c("c", "l", "l")
) %>%
  kable_styling(
    full_width = FALSE,
    bootstrap_options = c("striped", "hover", "condensed")
  )

```

Load the `rice_data.CSV` dataset yourself and store it as a dataframe called `rice`. Then answer the questions below.

```{webr}
#| exercise: ex_2.1.0
#| envir: Ex1
#| warning: false
#| message: false
# Load in the data

# Check the structure of the data 
```

```{webr}
#| exercise: ex_2.1.0
#| solution: true
#| envir: Ex1
rice <- read.csv("Data-sets/rice_data.csv")
str(rice)
```

```{webr}
#| exercise: ex_2.1.0
#| check: true
#| class: wait
#| envir: Ex1
gradethis::grade_this_code()
```

```{r, echo=FALSE}
rice <- read.csv("Data-sets/rice_data.csv",
                 header=TRUE) 
```

```{r, echo=FALSE, eval=FALSE}
str(rice)
```

1.  How many variables are in the `rice` dataset? `r fitb(23)`
2.  How many observations are in the `rice` dataset? `r fitb(1778)`
3.  How many categorical variables are there? (Hint: they may not be stored correctly...yet) `r fitb(12)`
4.  How many numerical variables are there? (Hint: they may not be stored correctly...yet) `r fitb(11)`
5.  What type of variable has R stored them? `r fitb("character", ignore_case = TRUE, ignore_ws = TRUE)`

You may notice that some data cleaning is required.

In the middle of the Excel file (around row 1465), there is a blank row, followed by a repeated header row.

```{r}
rice[1465:1466,]
```

In addition, there are several missing values in the dataset, which are denoted by "\*".

```{r}
sum(rice == "*", na.rm = TRUE)
```

There are a few different ways we can approach missing values. The first step is to identify the **extent of missing data** and then diagnose the issue. Is the data missing completely at random (MCAR), missing at random (MAR), or missing not at random (MNAR)? In other words, is the issue structural, or is it genuinely random?

Depending on the type of missing data, we might consider:

-   deleting rows with missing values

-   imputing missing values using the mean

-   imputing missing values using MICE (Multivariate Imputation by Chained Equations)

In these short courses, imputation is beyond the scope of the material covered, so we will focus on deleting missing rows (with appropriate justification). To learn how to properly treat missing data, I recommend the [*Missing Data Imputation in R*](https://libguides.princeton.edu/R-Missingdata) tutorial from Princeton.

Before exploring the missing data, let's do a quick clean. There are some trailing whitespace issues in a few observations, and we also want to focus on the \* entries and convert these to NA. We then filter out any blank rows and remove the repeated header row.

```{webr}
#| exercise: ex_2.1.0.1
#| envir: Ex1
#| warning: false
#| message: false
# Run the code below to clean the data.
library (dplyr)

clean_rice <- rice %>%
  # Convert "*" to NA
  mutate(across(where(is.character),
                ~ na_if(.x, "*"))) %>%
  # Drop fully blank rows
  filter(if_any(everything(), ~ !is.na(.x) & .x != "")) %>%  
  # Drop repeated header row
  filter(Sowing.method != "Sowing method")  
```

```{webr}
#| exercise: ex_2.1.0.1
#| solution: true
#| envir: Ex1
library (dplyr)

clean_rice <- rice %>%
  # Convert "*" to NA
  mutate(across(where(is.character),
                ~ na_if(.x, "*"))) %>%
  # Drop fully blank rows
  filter(if_any(everything(), ~ !is.na(.x) & .x != "")) %>%  
  # Drop repeated header row
  filter(Sowing.method != "Sowing method")  
```

```{webr}
#| exercise: ex_2.1.0
#| check: true
#| class: wait
#| envir: Ex1
gradethis::grade_this_code()
```

```{r, message=FALSE, echo=FALSE}
library (dplyr)

clean_rice <- rice %>%
  # Convert "*" to NA
  mutate(across(where(is.character),
                ~ na_if(.x, "*"))) %>%
  # Drop fully blank rows
  filter(if_any(everything(), ~ !is.na(.x) & .x != "")) %>%  
  # Drop repeated header row
  filter(Sowing.method != "Sowing method")  

```

Let's visualise the missing values and count how many cells are missing.

```{r}
sapply(clean_rice, function(x) sum(is.na(x)))
```

```{r}
sum(is.na(clean_rice))
```

```{r}
library(visdat)
vis_miss(clean_rice)
```

All treatment variables are complete, while missing values mostly occur in the response and intermediate agronomic measurements. The main example is HeaderYield, which is missing for many observations because machine-harvest data were only collected at some sites. In other words, this is structural missing data i.e. the values do not exist for certain site--season combinations rather than being randomly missing. Because of this, it makes sense to drop HeaderYield NAs only when we are visualising or analysing HeaderYield.

We also see missing values in the reference number (Ref). This appears to be caused by a data import issue, where a blank row and a repeated header row were introduced in the middle of the dataset. After this point, the reference numbers are no longer populated. Since Ref is just an identifier, filling in these values would be inappropriate, so we do not attempt to impute them. Instead, the problematic rows can be removed where needed.

Finally, for visualisation, we do not want to drop all rows that contain missing values, as this would remove valid data unnecessarily. Instead, for each plot, we only remove the missing values that are relevant to the variables being visualised.

We now continue cleaning the data accordingly.

```{webr}
#| exercise: ex_2.1.1
#| envir: Ex1
#| warning: false
#| message: false
# Load in tidyverse

# Load in stringr

# Tidy up the `clean_rice` dataset by filling in the blanks
clean_rice <- _______ %>%
  # Some observations contain trailing spaces, which causes identical values to be treated as different.
  # We remove these trailing spaces to avoid this issue.
  mutate(across(where(is.character), str_squish)) %>%
  ___________(______(
    c(
      Ref, Experiment.site, Year, Exp.name, Sowing.method,
      RANGE, ROW, REP, Variety, Nitrogen, N.group, Lodging..10.flat.
    ),
    __________
  )) %>%
  _______(across(where(is.character), as.numeric))
```

```{webr}
#| exercise: ex_2.1.1
#| solution: true
#| envir: Ex1
library(tidyverse)
library(stringr)

clean_rice <- clean_rice %>%
  mutate(across(where(is.character), str_squish)) %>%
  mutate(across(
    c(
      Ref, Experiment.site, Year, Exp.name, Sowing.method,
      RANGE, ROW, REP, Variety, Nitrogen, N.group, Lodging..10.flat.
    ),
    as.factor
  )) %>%
  mutate(across(where(is.character), as.numeric)) 
```

```{webr}
#| exercise: ex_2.1.1
#| check: true
#| class: wait
#| envir: Ex1
gradethis::grade_this_code()
```

```{r, echo = FALSE, message=FALSE}
library(tidyverse)
library(stringr)

clean_rice <- clean_rice %>%
  mutate(across(where(is.character), str_squish)) %>%
  mutate(across(
    c(
      Ref, Experiment.site, Year, Exp.name, Sowing.method,
      RANGE, ROW, REP, Variety, Nitrogen, N.group, Lodging..10.flat.
    ),
    as.factor
  )) %>%
  mutate(across(where(is.character), as.numeric)) 

```

Let's check the structure again to make sure everything has been cleaned up correctly.

```{webr}
#| exercise: ex_2.1.2
#| envir: Ex1
#| warning: false
#| message: false

```

```{webr}
#| exercise: ex_2.1.2
#| solution: true
#| envir: Ex1
str(clean_rice)
```

```{webr}
#| exercise: ex_2.1.2
#| check: true
#| class: wait
#| envir: Ex1
gradethis::grade_this_code()
```

Yay! Now that the dataset is cleaned, we're ready to start plotting.

### One Quantitative Variable

When you have a single numerical variable, whether discrete or continuous, a ***histogram*** is a great way to visualise its distribution.

In R, you can create plots using base R. For example, to see a visualisation of plant height, you can simply type:

```{r}
hist(clean_rice$Plant.height..cm.)
```

to generate a histogram.

While base R plots are fine for basic analysis,`ggplot2` is one of the most commonly used plotting packages and is widely accepted in published reports.

The histogram looks a bit plain without any color, and the x-axis label isn't very informative. Let's add some color, fix the labels, and give it a clean white background.

Load in the package and then run the code below.

```{r}
attach(clean_rice)
```

```{webr}
#| exercise: ex_2.2.1
#| envir: Ex1
#| warning: false
#| message: false
# load in ggplot2 

# run the code below
ggplot(data=clean_rice, aes(x=Plant.height..cm.))+
  geom_histogram(fill="palegreen3", col="darkgreen", bins=30)+
  labs(x="Rice Plant Height (cm)", y="Frequency")+
  theme_bw()
```

```{webr}
#| exercise: ex_2.2.1
#| solution: true
#| envir: Ex1
library(ggplot2)

ggplot(data=clean_rice, aes(x=Plant.height..cm.))+
  geom_histogram(fill="palegreen3", col="darkgreen", bins=30)+
  labs(x="Rice Plant Height (cm)", y="Frequency")+
  theme_bw()
```

```{webr}
#| exercise: ex_2.2.1
#| check: true
#| class: wait
#| envir: Ex1
gradethis::grade_this_code()
```

```{r, echo = FALSE, eval=FALSE, warning=FALSE, message=FALSE}
ggplot(clean_rice, aes(x = Plant.height..cm.)) +
  geom_histogram(fill = "palegreen3", col = "darkgreen", bins = 30) +
  labs(x = "Rice Plant Height (cm)", y = "Frequency") +
  theme_bw()
```

We can see that this is much better!

![](images/ggplothist.png)

If you see an NA warning when creating this histogram in RStudio, don't worry. R automatically excludes these values from the plot. To avoid the warning, we can explicitly drop the NAs for the variable in the pipeline, as shown below:

```{r}
clean_rice %>%
  drop_na(Plant.height..cm.) %>%
  ggplot(aes(x = Plant.height..cm.)) +
  geom_histogram(fill = "palegreen3", col = "darkgreen", bins = 30) +
  labs(x = "Rice Plant Height (cm)", y = "Frequency") +
  theme_bw()
```

It is very important that when creating a plot, you provide meaningful labels with units. You can also see below how different bin choices influence the histogram.

```{r, warning=FALSE, message=FALSE}
ggplot(data=clean_rice, aes(x=Plant.height..cm.))+
  geom_histogram(fill="palegreen3", col="darkgreen", bins=10)+
  labs(x="Rice Plant Height (cm)", y="Frequency")+
  theme_bw()
```

```{r, warning=FALSE, message=FALSE}
ggplot(data=clean_rice, aes(x=Plant.height..cm.))+
  geom_histogram(fill="palegreen3", col="darkgreen", bins=100)+
  labs(x="Rice Plant Height (cm)", y="Frequency")+
  theme_bw()
```

We can make the histogram even more informative by filling the bars by sowing method. Do plant heights differ between different sowing methods?

```{r, message=FALSE, warning = FALSE}
ggplot(data=clean_rice, aes(x=Plant.height..cm., fill=Sowing.method))+
  geom_histogram(binwidth=2)+
  labs(x="Rice Plant Height (cm)", y="Frequency")+ 
  theme_bw()
```

```{r, message=FALSE, warning = FALSE}
ggplot(data=clean_rice, aes(x=Plant.height..cm., col=Sowing.method))+
  geom_freqpoly(binwidth=2)+
  labs(x="Rice Plant Height (cm)", y="Frequency")+ 
  theme_bw()
```

```{r, message=FALSE, warning = FALSE}
ggplot(data=clean_rice, aes(x=Plant.height..cm., after_stat(density), col=Sowing.method))+
  geom_freqpoly(binwidth=2)+
  labs(x="Rice Plant Height (cm)", y="Frequency")+ 
  theme_bw()
```

  
  
### Two Quantitative Variables

When working with two quantitative variables, **scatterplot** is a suitable choice.

Fill in the blanks below to create a plot of dry matter at PI against plant height.

```{webr}
#| exercise: ex_2.2.2
#| envir: Ex1
#| warning: false
#| message: false
# Fill in the blanks
ggplot(______, aes(x=_______, y=__________))+
  geom_point(fill="darkolivegreen3", col="darkolivegreen", pch=21, size=3)+
  labs(x=expression("Weight of Dry Matter at PI ("*g/m^2*")"),y="Rice Plant Height (cm)")+
  geom_smooth(method="lm", col="black")+
  theme_bw()
```

```{webr}
#| exercise: ex_2.2.2
#| solution: true
#| envir: Ex1
ggplot(data=clean_rice, aes(x=PI.DM..g.msq., y=Plant.height..cm.))+
  geom_point(fill="darkolivegreen3", col="darkolivegreen", pch=21, size=3)+
  labs(x=expression("Weight of Dry Matter at PI ("*g/m^2*")"),y="Rice Plant Height (cm)")+
  geom_smooth(method="lm", col="black")+
  theme_bw()
```

```{webr}
#| exercise: ex_2.2.2
#| check: true
#| class: wait
#| envir: Ex1
gradethis::grade_this_code()
```

We can see that as the weight of the dry matter increases, the plant height also slightly increases. This suggests a *weak to moderate positive linear* relationship. Looking for more details on understanding linear relationships? Check out the *Inferential Statistics with R* and *Linear Regression* short courses!

To visualise the linear relationship, we include `method = "lm"` in `geom_smooth()`, which fits a linear model (a straight line) to the data.

```{r, warning=FALSE, message=FALSE}
ggplot(data=clean_rice, aes(x=PI.DM..g.msq., y=Plant.height..cm.))+
  geom_point(fill="darkolivegreen3", col="darkolivegreen", pch=21, size=3)+
  labs(x=expression("Weight of Dry Matter at PI ("*g/m^2*")"),y="Rice Plant Height (cm)")+
  geom_smooth(method="lm", col="black")+
  theme_bw()
```

We can customise the scatterplot even further by using the `mapping` argument to colour the points by sowing method.

```{r, warning=FALSE, message=FALSE}
clean_rice %>%
  drop_na(Sowing.method) %>%
  ggplot(aes(
    x = PI.DM..g.msq.,
    y = Plant.height..cm.,
    colour = Sowing.method,
    fill   = Sowing.method,
  )) +
  geom_point(size = 3) +
  labs(
    x = expression("Weight of Dry Matter at PI ("*g~m^-2*")"),
    y = "Rice Plant Height (cm)"
  ) +
  theme_bw()

```

![](images/ggplotscatter.png)

### Quantitative and Qualitative Variables

You may also want to create plots that compare quantitative and qualitative variables. For example, does plant height differ between rice varieties? How does header yield vary across sowing methods? In these cases, a **boxplot** is a useful way to visualise the relationship.

Fill in the blanks below to create a boxplot showing plant height (*Plant.height..cm.*) across groups representing different nitrogen rates (*N.group*).

```{webr}
#| exercise: ex_2.2.3
#| envir: Ex1
#| warning: false
#| message: false
# First, let's relevel the variable so the order is logical
clean_rice$N.group <- factor(
  clean_rice$N.group,
  levels = c("zero", "PW_1", "PW_2", "PW_3", "split", "split 2", "split 3")
)

# Fill in the blanks
ggplot(_____, aes(x=_____, y=_____))+
  geom_boxplot(fill="#EF8354", col="#DF3B57")+
  ___(x="Nitrogen Rate Group",y="Rice Plant Height (cm)")+
  theme_bw()
```

```{webr}
#| exercise: ex_2.2.3
#| solution: true
#| envir: Ex1
clean_rice$N.group <- factor(
  clean_rice$N.group,
  levels = c("zero", "PW_1", "PW_2", "PW_3", "split", "split 2", "split 3")
)

ggplot(clean_rice, aes(x=N.group, y=Plant.height..cm.))+
  geom_boxplot(fill="#EF8354", col="#DF3B57")+
  labs(x="Nitrogen Rate Group",y="Rice Plant Height (cm)")+
  theme_bw()
```

```{webr}
#| exercise: ex_2.2.3
#| check: true
#| class: wait
#| envir: Ex1
gradethis::grade_this_code()
```

```{r, echo=FALSE, eval=FALSE, warning=FALSE}
clean_rice$N.group <- factor(
  clean_rice$N.group,
  levels = c("zero", "PW_1", "PW_2", "PW_3", "split", "split 2", "split 3")
)
ggplot(clean_rice, aes(x=N.group, y=Plant.height..cm.))+
  geom_boxplot(fill="#EF8354", col="#DF3B57")+
  labs(x="Nitrogen Rate Group",y="Rice Plant Height (cm)")+
  theme_bw() 
```

You can also customise even further.

```{r, warning=FALSE}
ggplot(
  clean_rice,
  aes(x = N.group, y = Plant.height..cm., fill = N.group, colour = N.group)
) +
  geom_boxplot(col="black") +
  facet_wrap(~ Sowing.method) +
  scale_fill_manual(values =  c(
  "#D8A47F",
  "#EF8354",
  "#EE4B6A",
  "#C57A44",
  "#EA5C1F",
  "#EA1F44",
  "#0F7173")) +
  labs(
    x = "Nitrogen rate group",
    y = "Rice plant height (cm)",
    title = "Rice Plant Heights vs. Nitrogen Rate by Sowing Method"
  ) +
  theme_bw() +
  theme(legend.position = "none")

```

Try creating a boxplot of Header Yield (machine-harvested grain yield, tonnes per hectare at 14% moisture) versus sowing method on your own, and customise it to your liking.

`r hide("Show code")`

```{r}
clean_rice %>%
  drop_na(Header.Yield..t.ha..14..) %>%
ggplot(aes(x = Sowing.method, y = Header.Yield..t.ha..14.., fill = Sowing.method, 
                 colour = Sowing.method)) +
  geom_boxplot() +
  scale_fill_manual(values = c("#A05475", "#EE586C", "#6B5C8A")) +
  scale_colour_manual(values = c("#3B1F2B", "#D8162F", "#3F355F")) +
  labs(x = "Sowing Method", y = "Header Yield (Tonnes per Hectare*)",
       title="Sowing Method vs. Header Yield by Nitrogen Group",
       caption = "*At 14% Moisture") +
  theme_classic() +
  facet_wrap(~N.group)
```

`r unhide()`

```{r}
detach(clean_rice)
```

# Palmer Penguins

Now that we’ve learned a bit about plotting, let’s test your new skills using the `penguins` dataset we worked with in the previous chapter. Try completing the exercises below.

```{webr}
#| exercise: ex_2.3.1
#| envir: Ex1
#| warning: false
#| message: false
# 1. Load in `ggplot2`

# 2. Load in the `palmerpenguins` package

# 3. Load in the  `visdat` package and visualise the missing 
# values for the `penguins` dataset with vis_miss()
```

```{webr}
#| exercise: ex_2.3.1
#| solution: true
#| envir: Ex1
library(ggplot2)

library(palmerpenguins)

library(visdat)

vis_miss(penguins)


```

```{webr}
#| exercise: ex_2.3.1
#| check: true
#| class: wait
#| envir: Ex1
gradethis::grade_this_code()
```


```{r, echo=FALSE, message=FALSE}
library(ggplot2)

library(palmerpenguins)

library(visdat)
```

```{r, eval=FALSE, echo=FALSE}
vis_miss(penguins)
```

What do you notice about the missing data, and how might it be handled appropriately?

`r hide("Show hint")`

There is minimal missing data overall (approximately 0.7%). Although the variable sex has the highest proportion of missing values, the amount is small and does not show an obvious pattern with other variables in the dataset. As a result, removing these observations is unlikely to have a meaningful impact on the overall dataset.

`r unhide()`



```{webr}
#| exercise: ex_2.3.2
#| envir: Ex1
#| warning: false
#| message: false
# 1. Use na.omit() on penguins and name this cleaned dataset `clean_penguins`


# 2. Create a ggplot histogram of penguin body mass:
#   a. Fill the histogram with "orangered" and set the bins to 30
#   b. Title the x-axis "Body Mass  (g)"
#   c. Set the theme to black and white (bw).

```

```{webr}
#| exercise: ex_2.3.2
#| solution: true
#| envir: Ex1
clean_penguins <- na.omit(penguins)

ggplot(clean_penguins, aes(x=body_mass_g))+
  geom_histogram(fill="orangered", bins=30) +
  labs(x="Body Mass (g)")+
  theme_bw()
```

```{webr}
#| exercise: ex_2.3.2
#| check: true
#| class: wait
#| envir: Ex1
gradethis::grade_this_code()
```

```{r,echo=FALSE}
clean_penguins <- na.omit(penguins)
```

```{r, echo=FALSE, eval=FALSE}
ggplot(clean_penguins, aes(x=body_mass_g))+
  geom_histogram(fill="orangered", bins=30) +
  labs(x="Body Mass (g)")+
  theme_bw()
```


```{webr}
#| exercise: ex_2.3.3_1
#| envir: Ex1
#| warning: false
#| message: false
# 1. Create a frequency polygon of body mass.
#    Map body mass to the x-axis and colour the lines by species.
#    Use a bin width of 100, label the x-axis "Body Mass (g)" and the 
#    y-axis as "Frequency" and apply a black-and-white theme.

# 2. Create a frequency polygon of body mass scaled to density.
#    Map body mass to the x-axis, density to the y-axis using after_stat(density),
#    and colour the lines by species. 
#    Use a bin width of 100, 
#    Label the x-axis "Body Mass (g)" and the y-axis as "Frequency".

```

```{webr}
#| exercise: ex_2.3.3_1
#| solution: true
#| envir: Ex1

#1. 
ggplot(data=clean_penguins, 
       aes(x=body_mass_g, col=species))+ 
  geom_freqpoly(binwidth=100)+
  labs(x="Body Mass (g)", y="Frequency")+ 
  theme_bw()

#2. 
ggplot(data=clean_penguins, 
       aes(x=body_mass_g, 
           after_stat(density), 
           col = species)) + 
  geom_freqpoly(binwidth = 100)+
  labs(x="Body Mass (g)", y="Frequency")+ 
  theme_bw()

```

```{webr}
#| exercise: ex_2.3.3_1
#| check: true
#| class: wait
#| envir: Ex1
gradethis::grade_this_code()
```


```{r,echo=FALSE, eval=FALSE}
#1. 
ggplot(data=clean_penguins, 
       aes(x=body_mass_g, col=species))+ 
  geom_freqpoly(binwidth=100)+
  labs(x="Body Mass (g)", y="Frequency")+ 
  theme_bw()

#2. 
ggplot(data=clean_penguins, 
       aes(x=body_mass_g, 
           after_stat(density), 
           col = species)) + 
  geom_freqpoly(binwidth = 100)+
  labs(x="Body Mass (g)", y="Frequency")+ 
  theme_bw()
```


### Scatterplots

What if we're interested in how body mass relates to flipper length? In this case, we're working with two continuous numerical variables, so a **scatterplot** would be a good choice.


```{webr}
#| exercise: ex_2.3.3
#| envir: Ex1
#| warning: false
#| message: false
# 1. Create a scatterplot with flipper length on the x-axis and body mass on the y-axis.
#    Use filled points with shape 21, size 2.5, a fill colour of "aquamarine",
#    and an outline colour of "aquamarine4".
#    Label the axes as "Body Mass (g)" and
#    "Flipper Length (mm)", and apply a black-and-white theme.
```

```{webr}
#| exercise: ex_2.3.3
#| solution: true
#| envir: Ex1

ggplot(clean_penguins, aes(x=flipper_length_mm, y=body_mass_g))+
  geom_point(fill="aquamarine", col="aquamarine4", pch=21, size=2.5)+
  labs(x="Flipper Length (mm)", y="Body Mass (g)")+
  theme_bw()

```

```{webr}
#| exercise: ex_2.3.3
#| check: true
#| class: wait
#| envir: Ex1
gradethis::grade_this_code()
```

```{webr}
#| exercise: ex_2.3.4
#| envir: Ex1
#| warning: false
#| message: false
# 2. Create a scatterplot with flipper length on the x-axis and body mass on the y-axis.
#    Colour the points by species.
#    Add a linear regression line to the plot.
#    Label the axes as "Body Mass (g)" and
#    "Flipper Length (mm)", and apply a black-and-white theme.
```

```{webr}
#| exercise: ex_2.3.4
#| solution: true
#| envir: Ex1
ggplot(clean_penguins,
  mapping = aes(x=flipper_length_mm, y=body_mass_g)) +
  geom_point(mapping = aes(color = species)) +
  geom_smooth(method = "lm") +
  labs(x="Flipper Length (mm)", y="Body Mass (g)")+
  theme_bw()
```

```{webr}
#| exercise: ex_2.3.4
#| check: true
#| class: wait
#| envir: Ex1
gradethis::grade_this_code()
```

```{webr}
#| exercise: ex_2.3.5
#| envir: Ex1
#| warning: false
#| message: false
# 3. Modify the previous plot so that both colour and shape are mapped to species.
#    Add a linear regression line.
#    Manually set the colours with scale_colour_manual() for each species as follows:
#      Adelie = "#03045e"
#      Chinstrap = "#0096c7"
#      Gentoo = "#90e0ef"
#    Label the axes as "Body Mass (g)" and
#    "Flipper Length (mm)", and apply a black-and-white theme.

```

```{webr}
#| exercise: ex_2.3.5
#| solution: true
#| envir: Ex1
ggplot(
  data = clean_penguins,
  mapping = aes(x=flipper_length_mm, y=body_mass_g)
) +
  geom_point(mapping = aes(color = species, shape = species)) +
  geom_smooth(method = "lm") +
  scale_color_manual(
    values = c(
      Adelie = "#03045e",
      Chinstrap = "#0096c7",
      Gentoo = "#90e0ef"))+
  labs(x="Flipper Length (mm)", y="Body Mass (g)")+
  theme_bw()
```

```{webr}
#| exercise: ex_2.3.5
#| check: true
#| class: wait
#| envir: Ex1
gradethis::grade_this_code()
```


```{r, echo=FALSE, eval=FALSE, message=FALSE}

ggplot(clean_penguins, aes(x=flipper_length_mm, y=body_mass_g))+
  geom_point(fill="aquamarine", col="aquamarine4", pch=21, size=2.5)+
  labs(x="Flipper Length (mm)", y="Body Mass (g)")+
  theme_bw()

ggplot(clean_penguins,
  mapping = aes(x=flipper_length_mm, y=body_mass_g)) +
  geom_point(mapping = aes(color = species)) +
  geom_smooth(method = "lm") +
  labs(x="Flipper Length (mm)", y="Body Mass (g)")+
  theme_bw()
  
ggplot(
  data = clean_penguins,
  mapping = aes(x=flipper_length_mm, y=body_mass_g)
) +
  geom_point(mapping = aes(color = species, shape = species)) +
  geom_smooth(method = "lm") +
  scale_color_manual(
    values = c(
      Adelie = "#03045e",
      Chinstrap = "#0096c7",
      Gentoo = "#90e0ef"))+
  labs(x="Flipper Length (mm)", y="Body Mass (g)")+
  theme_bw()
```


### Boxplots

```{webr}
#| exercise: ex_2.3.5
#| envir: Ex1
#| warning: false
#| message: false
# Create a boxplot to compare body mass across penguin species.
#   Map species to the x-axis and body mass (in grams) to the y-axis.
#   Fill the boxes using the following colours, in order:
#     coral2, chartreuse2, cadetblue2
#   Set the outline colours to:
#     coral3, chartreuse4, cadetblue4
# Label the axes as "Species of Penguin" and "Body Mass (g)".


```

```{webr}
#| exercise: ex_2.3.5
#| solution: true
#| envir: Ex1
ggplot(clean_penguins, aes(x=species, y=body_mass_g))+
  geom_boxplot(fill = c("coral2", "chartreuse2", "cadetblue2"),
               col = c("coral3", "chartreuse4", "cadetblue4")) +
  labs(x="Species of Penguin", y="Body Mass (g)")
```

```{webr}
#| exercise: ex_2.3.5
#| check: true
#| class: wait
#| envir: Ex1
gradethis::grade_this_code()
```

Fantastic work! 


# Additional Notes 

```{r, warning = FALSE, message = FALSE}
ggplot(penguins, aes(x = flipper_length_mm, y = body_mass_g)) +
  geom_point(aes(color = species, shape = island)) +
  scale_colour_brewer(palette = "Set1")
  
```

```{r, warning = FALSE, message = FALSE}
ggplot(penguins, aes(x = flipper_length_mm, y = body_mass_g)) +
  geom_point(aes(color = species, shape = species)) +
  facet_wrap(~island) +
  scale_colour_viridis_d()

```

Colour blind

```{r, warning = FALSE, message = FALSE}
library(ggthemes)
ggplot(penguins, aes(x = flipper_length_mm, y = body_mass_g)) +
  geom_point(aes(color = species, shape = species)) +
  facet_wrap(~island) +
  scale_colour_colorblind()
```

```{r}
library("scales")

show_col(colorblind_pal()(8))
```
